...
func dataSourceAwsS3BucketObjects() *schema.Resource {
	return &schema.Resource{
		ReadContext: dataSourceAwsS3BucketObjectsRead,
...
func dataSourceAwsS3BucketObjectsRead(ctx context.Context, d *schema.ResourceData, meta interface{}) diag.Diagnostics {
	...
	if err != nil {
		return diag.FromErr(fmt.Errorf("error listing S3 Bucket (%s) Objects: %s", bucket, err))
	}

	if err := d.Set("common_prefixes", commonPrefixes); err != nil {
		return diag.FromErr(fmt.Errorf("error setting common_prefixes: %s", err))
	}
	...
	if err := d.Set("owners", owners); err != nil {
		return diag.FromErr(fmt.Errorf("error setting owners: %s", err))
	}
